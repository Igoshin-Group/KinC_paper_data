...
%%
wtt=[];at=[];dct=[];tt=3:0.2:9;ts=[3,4,5,6,7,8];
 wta=[];daa=[];dca=[];wtg=[];dag=[];dcg=[];for t=tt
   gr=gr_t(t);
   at=[at sum(fss_fit([2,3,4,5],gr,va,vc,vs,kg,kt,nt,p0e))];
    wtt=[wtt fss_fit([21],gr,va,vc,vs,kg,kt,nt,p0e)];
    dct=[dct fss_fit([21],gr,va,0,vs,kg,kt,nt,p0e)];
end
%%
figure;k=5487;
plot(log10(at),log10(wtt),'k-');hold on;plot(log10(at),log10(dct),'r-');hold on; 
legend('wt','dkinC');
xlabel('KinA()');
