...
tt=3:0.2:9;ts=[3,4,5,6,7,8];
 wta=[];oca=[];dca=[];wtg=[];dag=[];dcg=[];for t=tt
   gr=gr_t(t);
    wta=[wta fss_fit([11],gr,va*0,vc,vs,kg,kt,nt,p0e)];
    oca=[oca fss_fit([11],gr,va*0,vc*3,vs,kg,kt,nt,p0e)];
    dca=[dca fss_fit([11],gr,va*0,0,vs,kg,kt,nt,p0e)];
end
%%
tt=3:0.2:9;ts=[3,4,5,6,7,8];
plot(tt,wta,'k-');hold on;
plot(tt,oca,'r-');hold on;
plot(tt,dca,'b-'); hold on; xlim([3,8]);
legend(["wt","overexpress-kinc","delete-kinc"])
xlabel('time(h)');
ylabel('Spo0A~P(¦ÌM)');
title('kinA deleted')
%%